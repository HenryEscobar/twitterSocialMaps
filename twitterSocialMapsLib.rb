       ...
       if ( checkCache()  ) # non-nil return from checkCache
          ...
          if ( ( @status.downcase.match(/^wip$/) )    && 
               ( @heartbeat < Time.new.getutc.to_i )   )
                logMessage "#{msgPre}: WIP heartbeat expired. freeing"
                @status="ready"
          end # if wip

          case @status.downcase
            when ( /^done$/  )
               logMessage "#{msgPre}: done status.  Using Cached data"
               return 0
            when ( /^ready$/ )
               if ( options[:pollTwitter] ) 
                  logMessage "#{msgPre}: ready status. ask twitter"
                  loopTwitter 
               else
                  #logMessage "#{msgPre}: ready status. 
		  #    :pollTwitter: #{options[:pollTwitter]}"
                  return 0
               end
            when ( /^wip$/ )
               logMessage "#{msgPre}: wip status. worker: #{@workerName}"
               return 1
            else
               logMessage "ASSERT #{msgPre}: unknown status #{status}. exiting"
               exit 7
          end # case
       end # if checkCache

       logMessage "#{msgPre}: checkCache returned non-zero"
